...
//    }
//
//    func editUser(id: Int, user: newUser, success: @escaping (_ user: User) -> (),failure: @escaping (_ error: Error?) -> ()){
//
//        let url = "\(kbaseUrl)/users/\(id)"
//
//        AF.request(url, method: .put, parameters: user, encoder: JSONParameterEncoder.default).validate(statusCode: 200...299).responseDecodable (of: User.self){
//            response in
//            if let error = response.error {
//                failure(error)
//            } else if let user = response.value {
